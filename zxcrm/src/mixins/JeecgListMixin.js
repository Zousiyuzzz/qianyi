...
export const JeecgListMixin = {
  ...
  methods: {
    loadData(arg) {
      ...
      // 确保 url.list 是字符串类型
      let listUrl = this.url.list;
      if (typeof listUrl === 'object' && listUrl !== null && listUrl.method === 'post') {
        postAction(listUrl.url, params).then((res) => {
          if (res.success) {
            // // console.log(res)
            //update-begin---author:zhangyafei    Date:20201118  for：适配不分页的数据列表------------
            this.dataSource = res.result.records || res.result.result.records || res.result;
            if (res.result.total) {
        ...
        // 其他情况，显示错误信息
        this.$message.error("url.list 必须是字符串或对象类型");
        this.loading = false;
      }
    },
    initDictConfig() {
    ...
    getQueryParams() {
      ...
      var param = Object.assign(this.spppp || {}, sqp, this.queryParam, this.isorter, this.filters);
