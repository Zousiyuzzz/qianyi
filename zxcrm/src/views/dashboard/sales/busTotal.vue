...
export default {
    ...
    mounted() {
        this.$nextTick(() => {
            this.initChart();
            if (this.proCousumeList.length) {
                this.processChartData(this.proCousumeList);
            }
        });
    ...
    methods: {
        sumByProject(data) {
            ...
            data.forEach((item) => {
                const key = `${item.id}_${item.business_name || item.pro_name}`;
                const existing = resultMap.get(key);
                if (existing) {
                    existing.total_charged_in_yuan = parseFloat(
                        (existing.total_charged_in_yuan + item.total_charged_in_yuan).toFixed(3)
                    );
                } else {
                    resultMap.set(key, {
                        ...item,
                        total_charged_in_yuan: parseFloat(item.total_charged_in_yuan.toFixed(3)),
                    });
                }
            });
        ...
        processChartData(rawData) {
            ...
            this.chartData = this.sumByProject(rawData)
                .sort((a, b) => b.total_charged_in_yuan - a.total_charged_in_yuan)
                .map((item) => ({
                    name: item.business_name || item.pro_name,
