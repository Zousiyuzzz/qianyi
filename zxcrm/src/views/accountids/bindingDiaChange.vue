<template>
    <a-modal title="更改项目绑定" :visible="visible" :confirm-loading="confirmLoading" @ok="handleOk" @cancel="handleCancel"
...
export default {
    name: 'ProjectBindingChangeModal',
    components: {
        binding
    },
    data() {
        return {
            ...
            validatorRules: {
                ...
                date: [{
                    ...
                    //     } else if (this.earliestBindingTime && moment(value).isBefore(moment(this.earliestBindingTime), 'day')) {
                    //         callback(new Error(`绑定日期不能早于 ${this.earliestBindingTime}`))
                    //     } else {
                    //         callback()
                    //     }
                    // }
    ...
    methods: {
        ...
        disabledDate(current) {
            // 如果有最早的绑定时间，禁用早于该日期的日期（不包括这一天）
            if (this.earliestBindingTime && current) {
                const earliestDate = moment(this.earliestBindingTime)
                return current.isBefore(earliestDate, 'day')
            }
        ...
        // 显示模态框并初始化账号ID列表
        showModal(idlist, channel, earliestBindingTime) {
            this.Sign = channel
            this.dataList = idlist.map(item => item.accountId).filter(Boolean) || []
            this.earliestBindingTime = earliestBindingTime || null
            this.visible = true
        },
