        ...
        <div v-if="formDisabled" class="detail-view">
            <table class="detail-table">
                ...
                <tr>
                    ...
                    <td class="content">{{ model.fundsNature == 1 ? '对公' : '对私' }}</td>
                </tr>
                <tr>
                    <td class="label">项目主体</td>
                    <td class="content">{{ model.customerTitle }}</td>
                </tr>
                <tr>
                    <td class="label">我方主体</td>
                    <td class="content">{{ model.ourTitle }}</td>
                </tr>
                <tr>
                    <td class="label">备注</td>
                    <td class="content">
                        <div style="display: flex;flex-direction: column;gap: 10px;">
                            <div v-if="model.remarks && model.remarks.split('join').length > 1">{{
                                model.remarks.split('join')[0] || '-'
                                }}
                            </div>
                            <div v-if="model.remarks && model.remarks.split('join').length > 1">{{
                                model.remarks.split('join')[1] || '-'
                                }}
                            </div>
                            <div v-else>{{ model.remarks || '-' }}</div>
                        </div>
                    </td>
                </tr>
            </table>
        ...
        <j-form-container :disabled="formDisabled" v-else>
            <a-form-model ref="form" :model="model" :rules="validatorRules" slot="detail">
                <a-row>
                    ...
                    <!-- <a-col :span="24">
                        <a-form-model-item label="还款月份" :labelCol="labelCol" :wrapperCol="wrapperCol" prop="dateMonth">
        ...
        submitForm() {
            ...
            this.$refs.form.validate(valid => {
                if (valid) {
                    ...
                    let formData = {
                        ...this.model,
                        remarks: `超额退款备注:${this.model.chaoeremarks || '-'}join银行信息:${this.model.bankRemark || '-'}`
                    }
