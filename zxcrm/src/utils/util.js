...
function generateChildRouters(data) {
  ...
  for (let item of data) {
    ...
    // 检查组件路径是否包含无效值
    if (item.component === 'test' || item.component === 'undefined' || item.component === 'null') {
      continue;
    }
    ...
    } else if (item.component.indexOf("views/") >= 0) {
      // 如果已经是完整路径，直接使用
    ...
    if (item.component == "modules/online/cgform/OnlCgformHeadList") {
      ...
      if (!component || component.trim() === '') {
        componentPath = resolve => require(['@/views/jeecg/helloworld'], resolve)
      } else {
        componentPath = resolve => {
          try {
            // 检查组件路径是否包含无效字符
            if (component.includes('test') || component.includes('undefined') || component.includes('null')) {
              require(['@/views/jeecg/helloworld'], resolve)
              return
            }
